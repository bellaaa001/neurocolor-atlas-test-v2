const NAVIGATION = {
  ...
  renderStartPage(title, desc, action) {
    return `<section class="scientific-page"><h2>${title}</h2><p>${desc}</p><div class="button-container"><button class="btn-success" onclick="${action}">Begin</button><button class="btn-warning" onclick="NAVIGATION.skipToNextPage()">Skip Task</button><button class="btn-secondary" onclick="NAVIGATION.previousPage()">Back</button><button class="btn-secondary" onclick="NAVIGATION.pause()">Pause</button><button class="btn-danger" onclick="NAVIGATION.stopTest()">Stop Test</button></div></section>`;
  },
  askSkipReason() {
